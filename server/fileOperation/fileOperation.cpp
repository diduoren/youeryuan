...
#define PrintError(msg, err)	\
	KDebug("%s in (%s:%d, %s) errorcode=%d", msg, __FILE__, __LINE__, __FUNCTION__, err)
...
BEGIN_DLLINFO_MAP(HelpClass)
	ADD_DLLINFO_ENTRY(CONTROL_ENUM_DISKS, 10, "EnumAllDrives")
	ADD_DLLINFO_ENTRY(CONTROL_ENUM_FILES, 10, "EnumDir")
	ADD_DLLINFO_ENTRY(CONTROL_CREATE_DIR, 10, "CreateDir")
	ADD_DLLINFO_ENTRY(CONTROL_DELETE_FILE, 10, "DeleteFileOrDir")
	ADD_DLLINFO_ENTRY(CONTROL_RENAME_FILE, 10, "RenameFileOrDir")
	ADD_DLLINFO_ENTRY(CONTROL_UPLOAD_FILE, 10, "UploadFileOrDir")
	ADD_DLLINFO_ENTRY(CONTROL_DOWNLOAD_FILE, 10, "DownloadFileOrDir")
	ADD_DLLINFO_ENTRY(CONTROL_COPY_FILE, 10, "CopyFileOrDir")
	ADD_DLLINFO_ENTRY(CONTROL_RUN_FILE, 10, "ExecFile")
	ADD_DLLINFO_ENTRY(CONTROL_SEARCH_FILE, 10, "SearchFile")
...
{
	control_header header = INITIALIZE_COMPRESS_HEADER(CONTROL_ENUM_FILES);
...
{
	//control_header header = INITIALIZE_ENCYRPT_HEADER(CONTROL_CREATE_DIR);
...
{
	//control_header header = INITIALIZE_ENCYRPT_HEADER(CONTROL_RENAME_FILE);
	...
	oldFile = info->renameInfo.oldFile;
	newFile = info->renameInfo.newFile;
...
{
	//control_header header = INITIALIZE_ENCYRPT_HEADER(CONTROL_DELETE_FILE);
...
{
	//control_header header = INITIALIZE_ENCYRPT_HEADER(CONTROL_COPY_FILE);
	...
	oldfile = info->copyInfo.oldFile;
	newfile = info->copyInfo.newFile;
	attr = GetFileAttributesA(oldfile);
	if(attr == -1)//文件或者目录不存在
	{
		nStatus = ERR_NOTFIND;
		goto leave;
...
static unsigned __stdcall uploadThread(void* p)
{
	...
	while(1)
	{
		...
		{
			nStatus = conn_error; 
			//PrintError("read error %d", nStatus);
			goto leave; 
		}
		...
		case CONTROL_CREATE_DIR:
		{
			...
			if(conn_read(ftpfd, (char*)&info, sizeof(info), 0))
				...
				;//PrintError("read CONTROL_CREATE_DIR", conn_error);
			break;
		}
...
static int downloadFile(conn s, const char* fileName)
{
	...
	*/
	fileinfo.fileSizeHigh = /*size.HighPart*/high;
	fileinfo.fileSizeLow = /*size.LowPart*/low;
	...
	//发送文件具体数据
	int read_count = 0;
	__int64 allSize = 0, totalSize = ((__int64)high << 32) + low;
	char buf[1024] = {0};
	do{
...
int SearchFile(conn s, char* buf, int n, writeToClient fn)
{
	...
	char* findpattern = strlwr(info->searchInfo.pattern);
	const int MAX_PACKAGES = 4;
	struct dir_info dirs[MAX_PACKAGES];	
	int nContent = 0, nSeq = 0;
